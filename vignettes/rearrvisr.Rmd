...
Rearrangements are classified as nonsyntenic moves between compared genome segments (i.e., CARs), analogous to interchromosomal rearrangements (`NM1` and `NM2`), and as syntenic moves or inversions within compared genome segments, analogous to intrachromosomal rearrangements (`SM` and `IV`). `$NM1` stores class I `N`onsyntenic `M`oves; `$NM2` stores class II `N`onsyntenic `M`oves; `$SM` stores `S`yntenic `M`oves; `$IV` stores `I`n`V`ersions. See the Figure \ref{fig:classes} for examples of these four classes of rearrangements.
